    ...
    <div className="ring">
      <svg viewBox="0 0 70 70" aria-label="progress">
        ...
        <circle
          ...
          stroke="#667eea" strokeWidth="8" strokeLinecap="round"
          strokeDasharray={`${dash} ${circumference - dash}`}
          transform="rotate(-90 35 35)"
        />
        <text x="35" y="39" textAnchor="middle" fontSize="14" fontWeight="700" fill="#333">{p}%</text>
      </svg>
      <div>
...
export default function App() {
  ...
  const [chainParams, setChainParams] = useState<{ price: bigint; period: number; creator: string } | null>(null)
  const [currentBlock, setCurrentBlock] = useState<number>(0)
  const [subInfo, setSubInfo] = useState<{ active: boolean; expiry: number; now: number; hasPass: boolean } | null>(null)
  ...
  const connectWallet = async () => {
    try {
      ...
      setAllAccounts(accounts.map(acc => ({ address: acc.address, name: acc.meta.name as string })))
  ...
  const { result, output } = await contract.query.getParams(account, { gasLimit })
      ...
      if (result.isOk && output) {
        ...
        if (data && typeof data === 'object' && 'Ok' in data) {
          ...
          setContractInfo(`📋 Contract Settings: Price=${params[0]} plancks/period, Period=${params[1]} blocks, Creator=${params[4]}`)
        } else {
          setChainParams(null)
  ...
  const refreshSubscription = async () => {
    ...
    try {
      ...
      if ((contract.query as any).getSubscriptionInfo) {
        // @ts-ignore
        const { result, output } = await (contract.query as any).getSubscriptionInfo(account, { gasLimit }, account)
        if (result.isOk && output) {
          ...
          setSubInfo({ active: active === true || active === 'true', expiry: Number(String(expiry).replace(/,/g, '')), now: Number(String(now).replace(/,/g, '')), hasPass: hasPass === true || hasPass === 'true' })
          return
        }
      }
  ...
  const subscribe = async () => {
    ...
    try {
      ...
      if (value === 0n) {
        return setStatus('❌ Price cannot be 0. Enter amount in plancks (e.g., 1000000000000000000 = 1 SBY)')
      }
      ...
      if (balance < minRequired) {
        ...
        return setStatus(`❌ Insufficient balance. Have: ${balanceInSBY.toFixed(4)} SBY, Need: ~${required.toFixed(4)} SBY. Get more from faucet.`)
      }
      ...
      const { gasRequired, storageDeposit, result, output, debugMessage } = await contract.query.subscribe(
        account,
        { value, gasLimit }
      )
      ...
      console.log('Dry-run full result:', {
        ...
        flags: (result as any).flags?.toHuman ? (result as any).flags.toHuman() : (result as any).flags,
      ...
      const requiredRefTime = (gasRequired as any).refTime.toBigInt ? (gasRequired as any).refTime.toBigInt() : BigInt((gasRequired as any).refTime.toString().replace(/,/g, ''))
      const requiredProofSize = (gasRequired as any).proofSize.toBigInt ? (gasRequired as any).proofSize.toBigInt() : BigInt((gasRequired as any).proofSize.toString().replace(/,/g, ''))
      ...
      await tx.signAndSend(account, { signer: injector.signer as any, nonce }, ({ status: txStatus, dispatchError, events }) => {
        ...
        if (dispatchError) {
          ...
          console.log('Contract events:', contractEvents?.map(({ event }) => ({
            method: event.method,
            data: event.data.toHuman()
          })))
          ...
          if (contractEvent) {
            ...
            setTxHistory((prev) => [`Subscribed at #${txStatus.asFinalized.toString().slice(0, 10)}`, ...prev].slice(0, 10))
            refreshSubscription()
          } else {
            ...
            if (failedEvent) {
              ...
              setTxHistory((prev) => [`Failed at #${txStatus.asFinalized.toString().slice(0, 10)}`, ...prev].slice(0, 10))
            } else {
              setStatus(`✅ Transaction finalized in block: ${txStatus.asFinalized.toString().slice(0, 10)}...`)
              setTxHistory((prev) => [`Finalized #${txStatus.asFinalized.toString().slice(0, 10)}`, ...prev].slice(0, 10))
            }
          }
        } else {
  ...
  const updateContractParams = async () => {
    ...
    try {
      ...
      await tx.signAndSend(account, { signer: injector.signer as any, nonce }, ({ status, dispatchError }) => {
        if (dispatchError) {
          if (dispatchError.isModule) {
            const decoded = api.registry.findMetaError(dispatchError.asModule)
            setStatus(`❌ Error: ${decoded.section}.${decoded.name} - ${decoded.docs.join(' ')}`)
          ...
          checkContractParams()
          setTxHistory((prev) => [`Params updated at #${status.asFinalized.toString().slice(0, 10)}`, ...prev].slice(0, 10))
        }
      })
    } catch (e: any) {
      console.error(e)
    ...
    try {
      ...
      await tx.signAndSend(account, { signer: injector.signer as any, nonce }, ({ status, dispatchError }) => {
        if (dispatchError) {
          ...
          setTxHistory((prev) => [`Withdraw at #${status.asFinalized.toString().slice(0, 10)}`, ...prev].slice(0, 10))
  ...
  useEffect(() => { refreshSubscription(); refreshContractBalance() }, [api, metadata, contractAddress, account])
  ...
  return (
    <div className="app-container">
      <div className="app-card">
        ...
        <p className="subtitle">Subscribe to creators with on-chain micropayments. Zero platform fees.</p>
        ...
        <div className="form-group">
          ...
          <div className="input-with-btn">
            ...
            />
            <button className="btn btn-ghost btn-icon" onClick={copyAddress} title="Copy address">📋</button>
          </div>
          {contractInfo && (
          ...
          <div>
            <div className="form-group">
              ...
              <div className="form-hint">
                Quick fill: {[1,5,10].map((n) => (
                  <button key={n} className="btn btn-secondary btn-small" onClick={() => setPricePerPeriod(String(BigInt(n) * 1_000_000_000_000_000_000n))}>{n} SBY</button>
                ))}
              </div>
              {chainParams && (
                <div className="form-hint" style={{ marginTop: 8, color: '#0070f3' }}>
                  {(() => {
                    try {
                      ...
                      return (
                        <span>
                          Paying ≈ <strong>{periods}</strong> period(s) • adds ~<strong>{blocks}</strong> blocks ≈ <strong>{hours}h {mins}m</strong>
                        </span>
                      )
                    } catch {}
                    return null
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button 
                ...
                onClick={async () => { await subscribe(); notify('Subscription transaction submitted', 'info') }}
              ...
              <button 
                ...
                onClick={checkContractParams}
              >
                Check Contract
              </button>
            </div>

            <details className="card" style={{ background: '#fff3cd', border: '1px solid #ffc107' }}>
              <summary style={{ cursor: 'pointer', fontWeight: 'bold', color: '#856404' }}>🔧 Update Contract Settings (Creator Only)</summary>
              <div style={{ marginTop: 12 }}>
                ...
                <button 
                  ...
                  onClick={async () => { await updateContractParams(); notify('Contract settings updated', 'success') }}
                  disabled={isBusy || !account || !contractAddress}
                >
          ...
          <div>
              ...
              <div className="card card-info" style={{ marginBottom: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  {chainParams && subInfo?.active && (
                    <ProgressRing
                      percent={Math.max(0, Math.min(100, Math.round(((subInfo.expiry - currentBlock) / Math.max(1, chainParams.period)) * 100)))}
                ...
                <div style={{ marginTop: 12 }}>
                  <span className={`status-badge ${subInfo?.active ? 'status-badge-success' : 'status-badge-error'}`}>
            ...
            {contractAddress && (
              <div className="card">
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button className="btn btn-secondary" onClick={refreshContractBalance}>Refresh</button>
                  <button className="btn btn-primary" onClick={async () => { await withdraw(); notify('Withdraw sent', 'info') }} disabled={isBusy || (!!chainParams && chainParams.creator !== account)}>Withdraw</button>
            ...
            <div className="card" style={{ marginTop: 16 }}>
              ...
              {(subInfo?.active && subInfo?.expiry && (subInfo.expiry - currentBlock) * BLOCK_TIME_MS < 2 * 60 * 60 * 1000) && (
            ...
            {txHistory.length > 0 && (
              <div className="card" style={{ marginTop: 16 }}>
                ...
                <ul className="activity-list">
                  {txHistory.map((t, i) => <li key={i}>{t}</li>)}
                </ul>
              </div>
            )}
          </div>
        ...
        <details className="card" style={{ background: '#f5f5f5', marginTop: 16 }}>
          <summary style={{ cursor: 'pointer', fontWeight: 'bold' }}>ℹ️ How to use this app</summary>
          <ol style={{ marginTop: 12 }}>
            ...
            <li>Verify with <code>is_active(your_address)</code> in Polkadot.js Apps</li>
          </ol>
        </details>
        ...
        {showQr && (
          <div className="modal-backdrop" onClick={() => setShowQr(false)}>
            <div className="modal" onClick={(e) => e.stopPropagation()}>
              ...
              <img alt="QR Large" width={360} height={360} className="qr-image" src={`https://chart.googleapis.com/chart?chs=600x600&cht=qr&chl=${encodeURIComponent(contractAddress)}`} />
              <div style={{ marginTop: 8, display: 'flex', gap: 8 }}>
                <button className="btn btn-primary" onClick={copyAddress}>Copy Address</button>
                <a className="btn btn-secondary" href={`https://chart.googleapis.com/chart?chs=600x600&cht=qr&chl=${encodeURIComponent(contractAddress)}`} download="creator-direct-qr.png">Download QR</a>
              </div>
            </div>
          </div>
        )}
